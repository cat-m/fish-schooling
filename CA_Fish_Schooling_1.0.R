...
# uses round() in get.mean.dir()
# strong bias to cardinal directions
# forms small to medium clusters
# lots of gridlock
...
get.mean.dir <- function(slice,this.r,this.c) {
  ...
  mean <- signif(neighbor.directions/num.neighbors,0)
...
print <- function(slice) {
  ...
  # NW is PURPLE
  plot(x=nw.x,y=nw.y,ylim=c(1,size+4),
       xlim=c(1,size+4),pch=60,col="purple",
